...
import cn.imflowow.clickgui.components.CommandBox;
import cn.imflowow.clickgui.components.ModuleList;
	...
	public void keyTyped(char typedChar, int keyCode) throws IOException {
		if (this.onBindingModule != null) {
            if (keyCode == Keyboard.KEY_ESCAPE || keyCode == Keyboard.KEY_DELETE) {
				...
				MessageManager.addMessage(this.onBindingModule.cnName + "现在是未绑定的.", MessageManager.MessageType.Info, 3000);
            } else {
                this.onBindingModule.keyCode = keyCode;
                MessageManager.addMessage(this.onBindingModule.cnName
                        + " is now bound to \"" + Keyboard.getKeyName(keyCode) + "\".", MessageManager.MessageType.Info, 3000);
            }
